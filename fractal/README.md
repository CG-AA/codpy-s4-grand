...
1. **環境準備與虛擬環境建置**：
   * 在開始進行評測之前，請先確認使用 **Python 3.12** 版本，以避免發生版本衝突的問題。
   * 設定好 Python 的虛擬環境，首先建立虛擬環境：
     ```bash
     python -m venv venv
     ```
2. **進入工作目錄**：
   * 進入 `fractal` 目錄。
3. **依賴安裝**：
   * 假如還沒安裝環境配置，在終端機執行指令安裝必要的 Python 庫。
   * 在 Windows PowerShell 中無法直接使用 `&&`，多個命令連著執行需要使用分號分隔：
     ```powershell
...
5. **修改燒錄設定**：
   * 編輯 `burn_matrix.json`，將 `project_dir` 的路徑設定為指向你的實作目錄。
   * 例如：`...\CODPY-S4-GRAND\fractal\ESP32-FRAC-1`。  
6. **配置通訊埠**：
   * 打開 `render.py`。
   * 將第 8 行的 `SERIAL_PORT` 修改為你在步驟 4 紀錄的 COM Port。  
7. **燒錄與啟動**：  
...
**挑戰任務**：請找到 `ASSIGN_MODE == 3` 的 TODO 位置。

**實作引導：**

你需要在此處定義 Core 0 與 Core 1 如何分配工作。一個簡單且高效的嘗試是「奇偶行分派」：

* 讓 `core_id == 0` 的核心處理偶數行 (`y % 2 == 0`)。  
* 讓 `core_id == 1` 的核心處理奇數行 (`y % 2 == 1`)。
...
為了讓序列傳輸跟上雙核計算的速度，我們在 `render_segment` 中使用了 RLE 壓縮：
...
* **看門狗 (WDT)**：在長時間運算的 for 迴圈中，必須包含 `esp_task_wdt_reset()`，否則 ESP32 會認為程式當機而強制重啟。  
* **浮點運算優化**：在 `eval_pixel` 中，我們使用 `float` 進行複數運算以利用 ESP32 的硬體浮點運算單元 (FPU)。

## **四、任務**
> 這裡有三個碎形模板，請幫幫卡皮巴拉想想該怎麼調整分配模式才能達到最快的算繪效率吧！

#### 任務一
這會畫出一個燃燒船碎形，觀察看看怎麼分割和加速吧！（`-m` 後面是分配模式的參數，例：`-m 1`）
```bash
python .\render.py -t 2 -i 5000 -r -2.0 -1.2 1.0 1.2 -m 1
```

#### 任務二
這會畫出一個如同三角形的碎形，黑色區域的面積怎麼分割成一樣的比例呢？（`-m` 後面是分配模式的參數，例：`-m 1`）
```bash
python .\render.py -t 1 -i 2000 -cx -0.123 -cy 0.745 -n 3 -r -1 -0.8 1 1.2 -m 1
```

#### 任務三
這是曼德博集合的一個小島。（`-m` 後面是分配模式的參數，例：`-m 1`）
```bash
python .\render.py -t 0 -i 10000 -r -0.745417 0.117996 -0.744717 0.118696 -m 1
```

#### 任務四
這個碎形看起來像一張蜘蛛網！（`-m` 後面是分配模式的參數，例：`-m 1`）
```bash
python .\render.py -t 1 -i 15000 -cx -0.74546 -cy 0.11669 -r -0.575 -0.1 -0.3 0.15 -m 1
```
